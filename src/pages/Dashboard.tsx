...
const Dashboard = () => {
  ...
  const calculateMetrics = useMemo((): DashboardMetrics => {
    ...
    return {
      totalPurchases,
      grossWeight,
      // Agora representa o peso real do estoque atual
      totalSales,
      totalTransactions,
      totalExpenses,
      totalCashAdditions,
  ...
  return <div className="flex flex-col h-screen bg-slate-800">
      ...
      <header className="bg-slate-900 text-white p-4 border-b border-slate-700">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            {isAdminView ? <Button onClick={handleBackToAdmin} variant="ghost" size="sm" className="text-slate-400 hover:text-white">
                ...
                Admin
              </Button> : <Link to="/" className="flex items-center gap-2 text-slate-400 hover:text-white">
        ...
        {isAdminView && <div className="mt-3 flex items-center gap-2 bg-purple-600/20 px-3 py-2 rounded-lg">
            <Shield className="w-4 h-4 text-purple-400" />
            <span className="text-purple-300 text-sm">
              Visualizando: <strong>{adminViewingUserName}</strong>
            </span>
            <Button onClick={handleExitAdminView} variant="ghost" size="sm" className="ml-auto text-purple-300 hover:text-white text-xs">
              Sair
            </Button>
          </div>}
      </header>
      ...
      <main className="flex-1 p-4 md:p-6 overflow-auto">
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            {/* Fluxo de Caixa */}
            <Card className="bg-slate-700 border-slate-600 cursor-pointer hover:bg-slate-600/80 transition-all col-span-2" onClick={() => handleCardClick('daily-flow')}>
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
